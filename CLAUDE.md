...
fires at EVALUATION for a sha256-selected 10 % (its old sha256-derived delay made it inert —
0 of 672 committed verdict rows; verdicts before `BAND_CTL_RANDOM_CHANGED_ON` carry no evidence); `ctl_inverse_band` is computed on the
...
- **Forward cells carry a write-once `lag_{h}`.** The entry-band scorecard
  (`scorecard.outcome_series`) and the paper gate exclude `gapped`, `implausible` and `lag_unknown`
  cells symmetrically, never scoring a cell sampled after `LEDGER_MAX_CELL_LAG_S`; `ledger.summary()`
  and the dashboard print raw cells (no lag or plausibility filter — read them as diagnostics, not
  as scored evidence).
