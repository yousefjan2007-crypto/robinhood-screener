...
RATIONALE = (
    ...
    "bound), or when the TICK-LEVEL dark share over ARMED positions "
    "(livebook.live_stats_dict's flow_dark_share: sum(flow_dark_ticks) / sum(flow_ticks + "
    "flow_dark_ticks), summed over closes that ever armed) exceeds KILL_MAX_FLOW_DARK_SHARE, at "
    "which point the rule was mostly not running when it mattered and any difference measured is "
    "an artefact of the feature feed rather than of the hypothesis. A close-level count cannot "
    "detect this failure mode: 90% of closes never reaching 1.5x (so never arming, contributing "
    "zero dark ticks) with the 10% that do arm 100% dark is 0.10 of closes but 1.00 of armed "
    "ticks — measuring at tick level over armed positions is the only way the share reflects how "
    "often the rule ran blind rather than how often it never ran at all. The PAPER GATE (Phase 8) "
    "reads flow_dark_share directly and VOIDs this policy once it exceeds 0.50; n_flow_dark stays "
    "in live_stats_dict only as a backward-compatible per-close COUNT (armed closes whose own "
    "dark share exceeds 0.5), never the quantity a gate should read. "
