...
# Finest first. (timeframe, aggregate, label, approx_span_sec at limit=config.PATHS_LIMIT)
RESOLUTIONS = (
...
def _ohlcv(pool: str, timeframe: str, aggregate: int) -> tuple[str, list]:
    ...
    for b in bars:
        try:
            out.append({"ts": float(b["ts"]), "o": float(b["o"]), "h": float(b["h"]),
...
if __name__ == "__main__":
    import time
    # One live fetch on the canonical anchor: 1 pools call + at most 2 OHLCV calls at 0.25 Hz.
    ...
    try:
        ...
        p = {"status": DEFERRED, "res": None, "pool": None, "entry": 0.0, "bars": [], "n_before": 0}
    print(f"  status={p['status']}  res={p['res']}  pool={p['pool']}  n_bars={len(p['bars'])}  "
          f"n_before={p['n_before']}  entry={p['entry']:.4g}")
    if p["status"] == "ok":
